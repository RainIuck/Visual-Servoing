...
def get_difference(color_heightmap, color_space, bg_color_heightmap):
    ...
    # key_color_dist_prob = F.softmax(torch.from_numpy(key_color_dist), dim=0).detach().numpy()

    bg_key_color_dist = np.sqrt(np.sum(np.power(bg_color_heightmap - color_space,2), axis=3))
    # bg_key_color_dist_prob = F.softmax(torch.from_numpy(bg_key_color_dist), dim=0).detach().numpy()
