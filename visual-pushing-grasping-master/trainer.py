...
class Trainer(object):
    ...
    def forward(self, color_heightmap, depth_heightmap, is_volatile=False, specific_rotation=-1):
        ...
        if self.method == 'reactive':
            ...
            for rotate_idx in range(len(output_prob)):
                if rotate_idx == 0:
                    push_predictions = F.softmax(output_prob[rotate_idx][0], dim=1).detach().cpu().numpy()[:,0,(padding_width/2):(color_heightmap_2x.shape[0]/2 - padding_width/2),(padding_width/2):(color_heightmap_2x.shape[0]/2 - padding_width/2)]
                    grasp_predictions = F.softmax(output_prob[rotate_idx][1], dim=1).detach().cpu().numpy()[:,0,(padding_width/2):(color_heightmap_2x.shape[0]/2 - padding_width/2),(padding_width/2):(color_heightmap_2x.shape[0]/2 - padding_width/2)]
                else:
                    push_predictions = np.concatenate((push_predictions, F.softmax(output_prob[rotate_idx][0], dim=1).detach().cpu().numpy()[:,0,(padding_width/2):(color_heightmap_2x.shape[0]/2 - padding_width/2),(padding_width/2):(color_heightmap_2x.shape[0]/2 - padding_width/2)]), axis=0)
                    grasp_predictions = np.concatenate((grasp_predictions, F.softmax(output_prob[rotate_idx][1], dim=1).detach().cpu().numpy()[:,0,(padding_width/2):(color_heightmap_2x.shape[0]/2 - padding_width/2),(padding_width/2):(color_heightmap_2x.shape[0]/2 - padding_width/2)]), axis=0)
            ...
            for rotate_idx in range(len(output_prob)):
                if rotate_idx == 0:
                    push_predictions = output_prob[rotate_idx][0].detach().cpu().numpy()[:,0,int(padding_width/2):int(color_heightmap_2x.shape[0]/2 - padding_width/2),int(padding_width/2):int(color_heightmap_2x.shape[0]/2 - padding_width/2)]
                    grasp_predictions = output_prob[rotate_idx][1].detach().cpu().numpy()[:,0,int(padding_width/2):int(color_heightmap_2x.shape[0]/2 - padding_width/2),int(padding_width/2):int(color_heightmap_2x.shape[0]/2 - padding_width/2)]
                else:
                    push_predictions = np.concatenate((push_predictions, output_prob[rotate_idx][0].detach().cpu().numpy()[:,0,int(padding_width/2):int(color_heightmap_2x.shape[0]/2 - padding_width/2),int(padding_width/2):int(color_heightmap_2x.shape[0]/2 - padding_width/2)]), axis=0)
                    grasp_predictions = np.concatenate((grasp_predictions, output_prob[rotate_idx][1].detach().cpu().numpy()[:,0,int(padding_width/2):int(color_heightmap_2x.shape[0]/2 - padding_width/2),int(padding_width/2):int(color_heightmap_2x.shape[0]/2 - padding_width/2)]), axis=0)
    ...
    def backprop(self, color_heightmap, depth_heightmap, primitive_action, best_pix_ind, label_value):
        ...
        if self.method == 'reactive':
            ...
            loss_value = 0
            if primitive_action == 'push':
                # loss = self.push_criterion(self.model.output_prob[best_pix_ind[0]][0], torch.from_numpy(label).long().cuda())
                ...
                if self.use_cuda:
                    loss = self.push_criterion(self.model.output_prob[0][0], torch.from_numpy(label).long().cuda())
                else:
                    loss = self.push_criterion(self.model.output_prob[0][0], torch.from_numpy(label).long())
                loss.backward()
                loss_value = loss.item()

            elif primitive_action == 'grasp':
                # loss = self.grasp_criterion(self.model.output_prob[best_pix_ind[0]][1], torch.from_numpy(label).long().cuda())
                # loss += self.grasp_criterion(self.model.output_prob[(best_pix_ind[0] + self.model.num_rotations/2) % self.model.num_rotations][1], torch.from_numpy(label).long().cuda())

                # Do forward pass with specified rotation (to save gradients)
                push_predictions, grasp_predictions, state_feat = self.forward(color_heightmap, depth_heightmap, is_volatile=False, specific_rotation=best_pix_ind[0])

                if self.use_cuda:
                    loss = self.grasp_criterion(self.model.output_prob[0][1], torch.from_numpy(label).long().cuda())
                else:
                    loss = self.grasp_criterion(self.model.output_prob[0][1], torch.from_numpy(label).long())
                loss.backward()
                loss_value += loss.item()
            ...
            if primitive_action == 'push':
                ...
                if self.use_cuda:
                    loss = self.criterion(self.model.output_prob[0][0].view(1,320,320), torch.from_numpy(label).float().cuda()) * torch.from_numpy(label_weights).float().cuda()
                else:
                    loss = self.criterion(self.model.output_prob[0][0].view(1,320,320), torch.from_numpy(label).float()) * torch.from_numpy(label_weights).float()
                loss = loss.sum()
                loss.backward()
                loss_value = loss.item()
                ...
                if self.use_cuda:
                    loss = self.criterion(self.model.output_prob[0][1].view(1,320,320), torch.from_numpy(label).float().cuda()) * torch.from_numpy(label_weights).float().cuda()
                else:
                    loss = self.criterion(self.model.output_prob[0][1].view(1,320,320), torch.from_numpy(label).float()) * torch.from_numpy(label_weights).float()
                loss = loss.sum()
                loss.backward()
                loss_value = loss.item()
